...
class LLMClient {
    ...
    apiKey;
    constructor() {
        this.provider = env_1.env.LLM_PROVIDER.toLowerCase();
        this.apiKey = env_1.env.LLM_API_KEY;
    }
    /**
    ...
    async explainDecision(context) {
        ...
        if (!this.apiKey || this.apiKey.includes('sample')) {
            logger_1.logger.info('LLM API key not configured or in mock mode; using grounded deterministic explanation engine');
        ...
        try {
            ...
            logger_1.logger.warn('LLM API call failed, falling back to grounded explanation synthesis', err);
            return this.synthesizeGroundedExplanation(context);
        }
    }
    /**
...
- If information is unavailable, say so.`
     ...
     */
    buildStrictGroundingPrompt(context) {
...
Explain in 2-4 concise, professional sentences exactly why the system is in this state and why this dispatch mix was mathematically chosen.`;
     ...
     */
    synthesizeGroundedExplanation(context) {
        const state = context.currentState;
        const stage = context.shortfallStage ?? state?.shortfall_stage ?? types_1.ShortfallStage.STAGE_0_EARLY_WARNING;
        ...
        let explanation = '';
        if (stage === types_1.ShortfallStage.STAGE_0_EARLY_WARNING) {
        ...
        else if (stage === types_1.ShortfallStage.STAGE_1_PREEMPTIVE_PRECHARGE) {
        ...
        else if (stage === types_1.ShortfallStage.STAGE_2_DEFERRABLE_RESCHEDULE) {
        ...
        else if (stage === types_1.ShortfallStage.STAGE_3_EFFICIENT_DIESEL) {
        ...
        else if (stage === types_1.ShortfallStage.STAGE_4_FAIR_LOAD_SHEDDING) {
