...
exports.engineClient = exports.EngineClient = void 0;
...
class EngineClient {
    ...
    isMock;
    currentMockState;
    constructor(baseURL = env_1.env.ENGINE_BASE_URL, isMock = env_1.env.USE_MOCK_ENGINE) {
        this.isMock = isMock;
        ...
        if (this.isMock) {
            logger_1.logger.info('EngineClient initialized in MOCK MODE (Python engine simulated)');
        }
        else {
            logger_1.logger.info(`EngineClient initialized pointing to Python Engine at ${baseURL}`);
        }
    }
    setMockMode(enabled) {
        this.isMock = enabled;
    ...
    handleError(error, operationName) {
        if (axios_1.default.isAxiosError(error)) {
            ...
            if (axiosErr.code === 'ECONNREFUSED' || axiosErr.code === 'ENOTFOUND') {
                logger_1.logger.error(`Python engine connection refused during [${operationName}] at ${env_1.env.ENGINE_BASE_URL}`);
                throw new errors_1.EngineUnavailableError(`Python engine unreachable at ${env_1.env.ENGINE_BASE_URL}`);
            }
            if (axiosErr.code === 'ECONNABORTED' || axiosErr.message.includes('timeout')) {
                logger_1.logger.error(`Python engine request timed out during [${operationName}]`);
                throw new errors_1.EngineTimeoutError(`Python engine timed out during ${operationName}`);
            }
            if (axiosErr.response) {
                logger_1.logger.error(`Python engine returned error ${axiosErr.response.status} for [${operationName}]`, {
                    status: axiosErr.response.status,
                    data: axiosErr.response.data
                });
                throw new errors_1.EngineResponseError(`Engine error (${axiosErr.response.status}): ${JSON.stringify(axiosErr.response.data)}`, axiosErr.response.data);
            }
        }
        logger_1.logger.error(`Unexpected failure communicating with Python engine in [${operationName}]`, error);
        throw new errors_1.EngineUnavailableError(`Failed to communicate with optimization engine: ${error.message}`);
    }
    // ==========================================
     ...
     */
    async getLiveState(communityId = 'com-offgrid-01') {
        if (this.isMock) {
     ...
     */
    async getForecast(communityId = 'com-offgrid-01', horizonHours = 24) {
        if (this.isMock) {
    ...
    async optimize(payload) {
        ...
        try {
            const response = await this.client.post('/optimize', payload);
            return response.data;
        }
        catch (err) {
    ...
    async evaluateLadder(payload) {
        ...
        try {
            const response = await this.client.post('/ladder', payload);
            return response.data;
        }
        catch (err) {
     ...
     */
    async getRunway(communityId = 'com-offgrid-01') {
        if (this.isMock) {
     ...
     */
    async getSignal(communityId = 'com-offgrid-01') {
        if (this.isMock) {
    ...
    // ==========================================
    generateBaseMockState(communityId) {
        const now = new Date();
        ...
        let dieselKw = 0;
        let shortfallStage = types_1.ShortfallStage.STAGE_0_EARLY_WARNING;
        ...
        if (net >= 0) {
            ...
            if (deficit <= 50.0) {
                ...
                shortfallStage = types_1.ShortfallStage.STAGE_3_EFFICIENT_DIESEL;
                signalColor = 'YELLOW';
    ...
    getMockDispatchPlan(payload) {
        ...
        return {
            ...
            shortfall_stage: types_1.ShortfallStage.STAGE_0_EARLY_WARNING
        };
    }
    getMockLadderResponse(targetStage) {
        const stage = Math.min(Math.max(targetStage, 0), 4);
        const stageNames = [
    ...
    getMockRunwayForecast(communityId) {
        const daily = [];
        let remaining = 680.0;
        for (let d = 1; d <= 14; d++) {
    ...
    getMockScenarioResult(payload) {
        ...
        if (payload.scenario_type === 'WIND_DROP') {
            ...
            updated.shortfall_stage = types_1.ShortfallStage.STAGE_3_EFFICIENT_DIESEL;
            updated.signal = {
                color: 'YELLOW',
            ...
            updated.shortfall_stage = types_1.ShortfallStage.STAGE_1_PREEMPTIVE_PRECHARGE;
            costDelta = 12.0;
            co2Delta = 0.0;
...
exports.engineClient = new EngineClient();
